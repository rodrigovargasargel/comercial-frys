...
export default function GuiaSelladoraModal({ show, onHide, op, empresas }) {
  const [form, setForm] = useState({ empresa_id: '', nro_guia: '' })
  ...
  const handleGenerar = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/selladora/ops/${op.id}/trazabilidad`, {
        ...
        body: JSON.stringify({ cliente: clienteNombre, nro_guia: form.nro_guia })
      })
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      ...
      <Modal.Body>
        ...
        <Form.Group className="mb-3">
          <Form.Label>N° Guía <span className="text-danger">*</span></Form.Label>
          <Form.Control
            name="nro_guia"
            value={form.nro_guia}
